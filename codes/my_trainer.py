...
class Trainer:
    ...
    def _inner_training_loop(
        ...
        trans_loss = torch.tensor(0.0).to(args.device)
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                with self.accelerator.accumulate(model):
                    tr_loss_step, trans_loss_step, kd_loss_step = self.training_step(model, inputs)
                ...
                if (
                    ...
                    trans_loss += trans_loss_step
                    kd_loss += kd_loss_step
                ...
                if (
                    ...
                    self._maybe_log_save_evaluate(tr_loss, model, trial, epoch, ignore_keys_for_eval, trans_loss, kd_loss)
                else:
    ...
    def _maybe_log_save_evaluate(self, tr_loss, model, trial, epoch, ignore_keys_for_eval, trans_loss, kd_loss):
        if self.control.should_log:
            ...
            trans_loss_scalar = self._nested_gather(trans_loss).mean().item()
            kd_loss_scalar = self._nested_gather(kd_loss).mean().item()
            ...
            logs["trans_loss"] = round(trans_loss_scalar / (self.state.global_step - self._globalstep_last_logged), 4)
            logs["kd_loss"] = round(kd_loss_scalar / (self.state.global_step - self._globalstep_last_logged), 4)
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        return (
            ...
            outputs['trans_loss'].detach() / self.args.gradient_accumulation_steps,
            outputs['kd_loss'].detach() / self.args.gradient_accumulation_steps
        )
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        ...
        if not isinstance(self.model, supported_classes):
            ...
            # print('Here in _save_pretrained')
